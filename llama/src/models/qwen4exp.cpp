...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // Detect sidecar/draft-only model: has nextn layers but no trunk tensors.
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // Trunk layers: skip the nextn block (only for the MTP draft layer)
        if (il < n_layer) {
            continue;
        }

        // MTP draft-head tensors (only present on the last layer)
