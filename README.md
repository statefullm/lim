...
| `LIM_LATEX` | `1` | Set to `0` to disable inline KaTeX rendering in the browser viewer. Block-level `$$...$$` math still renders via `katex.renderToString`. Useful when `$HOME`, `$PATH`, and other env vars appear as bare text and get mangled by LaTeX processing. |
...
Each session writes a numbered log file in `$LIM_LOG_DIR/<N>`. With `LIM_DEBUG=1`, a companion token-level trace is written to `$LIM_LOG_DIR/<N>.tokens` showing every token fed into and generated by the model, with labels like `FEED SYSTEM_PROMPT_INIT`, `FEED USER_INPUT`, etc.
