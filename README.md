...
| `LIM_LLAMA_BUILD_DIR` | Use pre-built llama from external directory | `export LIM_LLAMA_BUILD_DIR=/path/to/llama/build` |
| `LIM_LLAMA_CMAKE_FLAGS` | Extra cmake flags passed to the llama build | `make LIM_LLAMA_CMAKE_FLAGS="-DGGML_AVX512=off"` |
...
| `LIM_MTP` | `0` | Set to `1` to enable MTP (multi-token-prediction) speculative decoding for models that ship a "nextn" draft head (Qwen3.5/3.8 family). A second context mirroring the main KV cache proposes up to `LIM_MTP_DRAFT` tokens per round, and the main model verifies all of them in a single batch decode; each committed token is a fresh sample from the full target sampler chain at its own verify position, so the output is distribution-exact under any sampling configuration. It is not token-identical to a non-MTP run (the one-forward verify decode is not bit-identical to sequential decodes), but two MTP runs at `LIM_TEMPERATURE=0` are token-identical to each other. The GGUF is probed at startup and MTP is disabled without it; the draft context's memory (KV grows with `LIM_CTX`, compute buffers with `LIM_MTP_BATCH`) is printed at startup with `LIM_DEBUG=1` (a failed draft-context allocation prints the KV estimate unconditionally); budget it against your VRAM headroom, and lower `LIM_CTX` or `LIM_MTP_BATCH` if it doesn't fit. Speculation pauses (never crashes) when the mirror can no longer match the main context -- instant `/undo`, tool-correction rollback, fast-restore `/load` -- and re-enables on `/clear`, slow restore, or a full re-decode ("mirror cleared -- speculative decoding enabled"). It auto-disables itself when committed tokens/round stays below 1.25 for two consecutive 32-round windows (the draft overhead is not paying for itself); `/clear` re-enables it. Benchmark chatbot modes 1/2 are excluded. |
