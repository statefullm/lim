    ...
    while (true) {
        ...
        if (trigger_tool_execution && tool_start != string::npos && tool_end != string::npos) {
            ...
            auto tool_result = ToolExecutor::execute(
                state, generated_text, full_generated,
                tool_start, tool_end, was_mid_tool_call,
                tokenize, feed_tokens,
                ctx, n_past, cparams,
                g_auto_continue_depth, max_auto_continue,
                allow_continue_resume
            );
